    ...
    public static String str2Hex(String str) {
        ...
        if (str != null && !"".equals(str)) {
            ...
            for (int i = 0; i < bytes.length; i++) {
                byte b = bytes[i];
                int byteH = (b & BYTE_H) >> 4, byteL = (b & BYTE_L);
                builder.append(hexChars[byteH]).append(hexChars[byteL]);
            }
    ...
    public static String hex2Str(String hexStr) {
        ...
        if (hexStr != null && !"".equals(hexStr)) {
            ...
            for (int i = 0; i < orgin.length; i++) {
                int bit = (HEX_SEQ.indexOf(hexChars[2 * i]) << 4 | HEX_SEQ
                        .indexOf(hexChars[2 * i + 1]));
                orgin[i] = (byte) (bit & BYTE_MAX);
            }
